...
import { NavigationNode, AttachmentPreset } from "@shared/types";
...
class DocumentScene extends React.Component<Props> {
  ...
  handleCoverImg = async (e: React.ChangeEvent<HTMLInputElement>) => {
    this.coverImg =
      e?.target?.files !== null
        ? URL.createObjectURL(e?.target?.files[0])
        : this.coverImg;
    this.props.document.coverImg =
      e?.target?.files !== null
        ? URL.createObjectURL(e?.target?.files[0])
        : this.coverImg;
    this.updateIsDirty();
  ...
  render() {
    ...
    const handleUpdatePostion = async (
      positionX: number,
      positionY: number
    ) => {
    ...
    const handleCoverPosition = async (
      positionY: number,
      positionX: number
    ) => {
